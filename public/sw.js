...
async function dismissRoomNotification(roomId) {
  if (!roomId) return;
  const notifications = await self.registration.getNotifications({ tag: notificationTag(roomId) });
...
async function writePushConfig(value) {
  ...
  await new Promise((resolve, reject) => {
    ...
    if (value) store.put(value, "config");
    else store.delete("config");
...
self.addEventListener("push", (event) => {
  ...
  event.waitUntil((async () => {
    ...
    if (!visible) {
      operations.push(self.registration.showNotification("Sub-Etha", {
        body: test ? "The test transmission arrived successfully." : "A new transmission has arrived.",
        icon: "/icon-192.png",
        badge: "/icon-192.png",
