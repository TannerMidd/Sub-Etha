...
export function SubEthaApp() {
  ...
  const connect = useCallback(async (session: PersistedMatrixSession) => {
    ...
    const { MatrixAlreadyOpenError, MatrixService: MatrixServiceImplementation } = await import("@/lib/matrix/client");
    ...
    try {
      ...
      nextService.stop();
      if (error instanceof MatrixAlreadyOpenError) setBootState("duplicate");
      else {
  ...
  useEffect(() => {
    ...
    const watchInstalling = () => {
      ...
      const changed = () => {
        if (installing.state === "installed" && navigator.serviceWorker.controller) setWaitingWorker(registration?.waiting ?? null);
      };
  ...
  if (bootState === "connected" && service) return <>
    ...
    {waitingWorker ? <div className="update-toast" role="status"><RefreshCw /><span><strong>A revised field guide is ready.</strong><small>Update when you have finished composing anything important.</small></span><button type="button" onClick={applyUpdate}>Update now</button></div> : null}
  </>;
    ...
    return (
      <main className="boot-screen">
        ...
        <div className="boot-card">
          ...
          <p>To protect your encryption store, only one tab may tune this account at once. You may release the other receiver and continue here.</p>
          <div className="button-row"><button className="primary-button" type="button" onClick={() => void takeOver()}><RadioTower />Use this tab</button><button className="secondary-button" type="button" onClick={() => window.location.reload()}><RefreshCw />Try again</button></div>
        </div>
      </main>
    );
  }
    ...
    return (
      <main className="boot-screen">
        ...
        <div className="boot-card boot-card--error">
          ...
          <p>{bootError}</p>
          <div className="button-row"><button className="primary-button" type="button" onClick={() => window.location.reload()}><RefreshCw />Try again</button><button className="secondary-button" type="button" onClick={() => setBootState("login")}>Return to sign in</button></div>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="boot-screen" aria-busy="true">
      <BrandMark />
      <div className="tuning-indicator" aria-live="polite"><span><LoaderCircle className="spin" /></span><p>Aligning local encryption, room indexes and a modest number of invisible antennas…</p></div>
    </main>
  );
}
