...
const BUILT_IN_PUSH_HOSTS = new Set(["fcm.googleapis.com", "updates.push.services.mozilla.com"]);
...
function allowedPushHost(host: string, additionalHosts: Iterable<string>): boolean {
    if (BUILT_IN_PUSH_HOSTS.has(host)) {
        return true;
    }

    if (host.endsWith(".push.apple.com") || host.endsWith(".notify.windows.com")) {
        return true;
    }
...
export function validPushEndpoint(
    raw: unknown,
    additionalHosts: Iterable<string> = configuredPushHosts(),
): raw is string {
...
export function createPublicLookup(
    resolver: PushHostResolver = systemPushResolver,
): LookupFunction {
    return (hostname, options, callback) => {
        resolver(hostname, (error, addresses) => {
            ...
            if (!addresses.length || addresses.some(({ address }) => !isPublicIpAddress(address))) {
                callback(
                    lookupError(
                        "Push endpoint did not resolve exclusively to public addresses.",
                        "EACCES",
                    ),
                    options.all ? [] : "",
                    0,
            ...
            const requestedFamily =
                options.family && options.family !== 0 ? Number(options.family) : 0;
            ...
            if (!matching.length) {
                callback(
                    lookupError(
                        "Push endpoint did not resolve for the requested address family.",
                        "EAI_ADDRFAMILY",
                    ),
                    options.all ? [] : "",
                    0,
...
export function genericNotificationPayload(
    ...
    return JSON.stringify({
        ...
        unread: Number.isFinite(notification.counts?.unread)
            ? Math.max(0, Number(notification.counts?.unread))
            : 0,
    });
}
