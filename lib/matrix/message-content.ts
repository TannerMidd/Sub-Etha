...
export function createMediaContent(options: {
  ...
  if (caption) content.filename = options.fileName;
  if (options.replyTo) content["m.relates_to"] = { "m.in_reply_to": { event_id: options.replyTo } };
  if (options.encryptedFile) content.file = { ...options.encryptedFile, url: options.contentUri };
  else content.url = options.contentUri;
  return content;
}
