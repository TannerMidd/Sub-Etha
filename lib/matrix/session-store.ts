...
function openDatabase(): Promise<IDBDatabase> {
    return new Promise((resolve, reject) => {
        ...
        request.onerror = () =>
            reject(request.error ?? new Error("Could not open private session storage."));
    });
}
...
async function transact<T>(
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => resolve(request.result);
        request.onerror = () =>
            reject(request.error ?? new Error("Private session storage failed."));
        transaction.oncomplete = () => database.close();
        transaction.onerror = () =>
            reject(transaction.error ?? new Error("Private session transaction failed."));
    });
}
...
export function createSession(
    input: Omit<PersistedMatrixSession, "cryptoStorageKey">,
