...
const LINK_PATTERN = /(?:(?:https?|ftp):\/\/|www\.|magnet:\?)[^\s<>"']+|mailto:[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}|[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi;
...
function hrefFor(value: string): string {
  ...
  if (/^(?:mailto|magnet):/i.test(value)) return value;
...
export function messageTextSegments(value: string): MessageTextSegment[] {
  ...
  for (const match of value.matchAll(LINK_PATTERN)) {
    ...
    if (index > cursor) pushText(segments, value.slice(cursor, index));
    const [link, suffix] = splitTrailingPunctuation(match[0]);
    if (link) segments.push({ text: link, href: hrefFor(link) });
    pushText(segments, suffix);
    cursor = index + match[0].length;
  }
  if (cursor < value.length) pushText(segments, value.slice(cursor));
  return segments;
}
...
export function stripPlainReplyFallback(value: string): string {
  ...
  while (firstContentLine < lines.length && lines[firstContentLine].startsWith("> ")) firstContentLine += 1;
  if (firstContentLine === 0) return value;
  if (lines[firstContentLine] === "") firstContentLine += 1;
