...
export function assertDeclaredMediaLimits(media: { size?: number; width?: number; height?: number }): void {
  if (typeof media.size === "number") assertMediaByteLength(media.size);
  if (typeof media.width === "number" && (!Number.isFinite(media.width) || media.width <= 0 || media.width > MAX_IMAGE_EDGE)) {
    throw new MediaLimitError("This image exceeds Sub-Etha's automatic preview dimensions.");
  }
  if (typeof media.height === "number" && (!Number.isFinite(media.height) || media.height <= 0 || media.height > MAX_IMAGE_EDGE)) {
    throw new MediaLimitError("This image exceeds Sub-Etha's automatic preview dimensions.");
  }
  if (typeof media.width === "number" && typeof media.height === "number" && media.width * media.height > MAX_IMAGE_PIXELS) {
...
function timedRead<T>(operation: Promise<T>, timeoutMs: number, signal?: AbortSignal): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    ...
    const onAbort = () => {
      cleanup();
      reject(signal?.reason ?? new DOMException("The media download was aborted.", "AbortError"));
    };
    const cleanup = () => {
...
export async function readBoundedResponse(
  ...
  try {
    while (true) {
      ...
      if (remaining <= 0) throw new MediaTimeoutError();
      let result: ReadableStreamReadResult<Uint8Array>;
      try {
        result = await timedRead(reader.read(), Math.min(idleTimeoutMs, remaining), options.signal);
      } catch (error) {
...
function skipGifSubBlocks(bytes: Uint8Array, start: number): number {
  ...
  while (offset < bytes.byteLength) {
    ...
    if (offset + size > bytes.byteLength) return bytes.byteLength;
    offset += size;
  }
  return offset;
}
...
function gifFrameCount(bytes: Uint8Array): number {
  if (!bytesAreGif(bytes) || bytes.byteLength < 13) return 0;
  let offset = 13;
...
function pngFrameCount(bytes: Uint8Array): number {
  if (bytes.byteLength < 8 || !bytes.slice(0, 8).every((value, index) => value === [137, 80, 78, 71, 13, 10, 26, 10][index])) {
    return 0;
  }
  ...
  while (offset + 12 <= bytes.byteLength) {
    ...
    if (type === "acTL" && length >= 8) return view.getUint32(offset + 8);
    offset += 12 + length;
  }
  return 1;
}
...
function webpFrameCount(bytes: Uint8Array): number {
  ...
  while (offset + 8 <= bytes.byteLength) {
    ...
    if (length > bytes.byteLength - offset - 8) break;
...
function imageFrameCount(bytes: Uint8Array): number {
  if (bytesAreGif(bytes)) return Math.max(1, gifFrameCount(bytes));
  const pngFrames = pngFrameCount(bytes);
  if (pngFrames > 0) return pngFrames;
...
function safeRasterMimeType(bytes: Uint8Array): ImageSafety["mimeType"] {
  if (bytesAreGif(bytes)) return "image/gif";
  if (
    bytes.byteLength >= 8
...
export function assertSafeImageBytes(bytes: Uint8Array): ImageSafety {
  ...
  if (frameCount > Math.floor(MAX_IMAGE_PIXELS / pixels)) {
    throw new MediaLimitError("This animation exceeds Sub-Etha's automatic preview decode limit.");
  }
