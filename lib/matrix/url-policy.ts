...
export function assertAllowedHomeserverUrl(raw: string, policy: HomeserverUrlPolicy = {}): string {
  ...
  if (production || !appUrl || !isLoopbackHostname(appUrl.hostname) || !isLoopbackHostname(url.hostname)) {
    throw new InsecureHomeserverError("Matrix homeservers must use HTTPS outside loopback development.");
  }
