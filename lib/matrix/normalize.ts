...
function reactionsFor(eventId: string, reactions: ReturnType<typeof reactionMap>): ReactionSummary[] {
  return [...(reactions.get(eventId)?.entries() ?? [])]
    .map(([key, value]) => ({ key, ...value }))
    .sort((left, right) => right.count - left.count || left.key.localeCompare(right.key));
}

function classifyMessage(content: Record<string, unknown>, event: MatrixEvent): TimelineItem["type"] {
  if (event.isDecryptionFailure()) return "encrypted";
  if (event.getType() === EventType.RoomMember) return "system";
  const messageType = content.msgtype;
  if (messageType === "m.image") return "image";
  if (messageType === "m.video") return "video";
  if (messageType === "m.audio") return "audio";
  if (messageType === "m.file") return "file";
  if (messageType === "m.notice") return "notice";
  return "message";
}
...
function membershipBody(event: MatrixEvent, senderName: string): string {
  ...
  if (membership === "join") return `${senderName} joined the room.`;
  if (membership === "leave") return `${senderName} left the room.`;
  if (membership === "invite") return `${senderName} sent an invitation.`;
  if (membership === "ban") return `${senderName} was banned.`;
...
export function normalizeTimeline(room: Room, client: MatrixClient): TimelineItem[] {
  ...
  const visible = events.filter((event) => {
    ...
    return event.getType() === EventType.RoomMessage || event.getType() === "m.room.encrypted" || event.getType() === EventType.RoomMember;
  });
  ...
  return visible.map((event) => {
    ...
    const content = (replacingContent?.["m.new_content"] ?? original["m.new_content"] ?? original) as Record<string, unknown>;
    const relation = content["m.relates_to"] as { "m.in_reply_to"?: { event_id?: string } } | undefined;
    const eventId = event.getId() ?? `local-${event.getTs()}-${senderId}`;
    const mediaFile = content.file && typeof content.file === "object" ? content.file as Record<string, unknown> : undefined;
    const mxcUrl = typeof content.url === "string" ? content.url : typeof mediaFile?.url === "string" ? mediaFile.url : undefined;
    const readBy = eventId && event.getSender() === ownUserId
      ? room.getMembers()
        .filter((candidate) => candidate.userId !== ownUserId && room.hasUserReadEvent(candidate.userId, eventId))
...
export function sortRoomSummaries(rooms: RoomSummary[]): RoomSummary[] {
  return [...rooms].sort((left, right) => {
      if (left.membership !== right.membership) return left.membership === "invite" ? -1 : 1;
