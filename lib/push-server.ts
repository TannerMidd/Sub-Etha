...
export function pushLimitsFromEnvironment(environment: PushEnvironment): PushLimits {
    return {
        ...
        deliveriesPerMinute: positiveIntegerEnvironment(
            environment,
            "PUSH_DELIVERY_LIMIT_PER_MIN",
            3_000,
        ),
    };
}
...
export function createPushServer(dependencies: PushServerDependencies = {}) {
    ...
    const configured = (): PushConfiguration | Response =>
        configuration() ?? json({ error: "Web Push is not configured." }, 503);
    ...
    const consumeBudget = async (
        ...
        const allowed = await repository.consumeGlobalRateLimit(
            bucket,
            now(),
            windowSeconds,
            limit,
        ...
        if (!allowed) {
            log({ route: "push-budget", status: 429, budgetCategory: bucket, allowed: false });
        }
    ...
    const cleanupStaleSubscriptions = async (): Promise<number> => {
        const removed = await repository.cleanupSubscriptions(
            now() - STALE_SUBSCRIPTION_SECONDS,
            STALE_CLEANUP_BATCH,
        );

        if (removed > 0) {
            log({ route: "subscription-cleanup", status: 200, removedCount: removed });
        }
    ...
    const deliver = async (
        ...
        if (
            ...
            !(await repository.claimDelivery(
                ...
                eventId,
                timestamp,
                PENDING_DELIVERY_TIMEOUT_SECONDS,
            ))
        ) {
            return "suppressed";
        }
        ...
        if (
            !(await consumeBudget(
                BUDGET_OUTBOUND_DELIVERIES,
                ONE_MINUTE_SECONDS,
                limits.deliveriesPerMinute,
            ))
        ) {
            if (eventId) {
    ...
    return {
        ...
        changeSubscription(request: Request): Promise<Response> {
            return guarded("subscriptions", async () => {
                ...
                if (request.method === "PATCH") {
                    ...
                    if (!validPushKey(body.challenge)) {
                        return json({ error: "Invalid push confirmation." }, 400);
                    }
                ...
                if (request.method === "DELETE") {
                    ...
                    if (!validPushKey(body.managementKey)) {
                        return json({ error: "Invalid push management key." }, 400);
                    }
                ...
                if (p256dh.length > 256 || auth.length > 128) {
                    return json({ error: "Invalid push subscription keys." }, 400);
                }
                ...
                if (outcome === "management_conflict") {
                    return json(
                        {
                            error: "The delivery identifier is already managed by another browser capability.",
                        },
                        409,
                    );
                }
                ...
                try {
                    ...
                    return json(
                        { error: "The browser push endpoint could not be confirmed." },
                        502,
                    );
                }
            });
        ...
        notify(request: Request): Promise<Response> {
            return guarded("matrix-notify", async () => {
                ...
                if (
                    !(await consumeBudget(
                        BUDGET_MATRIX_NOTIFY,
                        ONE_MINUTE_SECONDS,
                        limits.notifyPerMinute,
                    ))
                ) {
                    return json(
