...
test("Web Push endpoints are restricted to recognized browser providers", () => {
  ...
  assert.equal(validPushEndpoint("https://push.custom.example/device", ["push.custom.example"]), true);
...
function runLookup(resolver: PushHostResolver, options: { all?: boolean; family?: number } = {}): Promise<{
...
test("connect-time lookup rejects private and mixed DNS answers", async () => {
  const privateOnly = await runLookup((_hostname, callback) => callback(null, [
    { address: "10.0.0.4", family: 4 },
  ]));
  assert.equal(privateOnly.error?.code, "EACCES");

  const mixed = await runLookup((_hostname, callback) => callback(null, [
    { address: "93.184.216.34", family: 4 },
    { address: "127.0.0.1", family: 4 },
  ]));
  assert.equal(mixed.error?.code, "EACCES");

  const publicOnly = await runLookup((_hostname, callback) => callback(null, [
    { address: "93.184.216.34", family: 4 },
...
test("invalid push limit overrides fail closed to documented defaults", () => {
  assert.deepEqual(pushLimitsFromEnvironment({
