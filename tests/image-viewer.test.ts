...
function assertSize(
    ...
    assert.ok(Math.abs(actual.width - expected.width) < 0.001, `${actual.width} should equal ${expected.width}`);
    assert.ok(Math.abs(actual.height - expected.height) < 0.001, `${actual.height} should equal ${expected.height}`);
}

test("fit-first sizing contains wide, tall, square, and small images without upscaling", () => {
    assertSize(containImageSize(
