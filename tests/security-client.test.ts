...
test("homeserver transport requires HTTPS outside loopback development", () => {
  ...
  assert.equal(assertAllowedHomeserverUrl("http://localhost:8008", LOCAL_POLICY), "http://localhost:8008");
  assert.equal(assertAllowedHomeserverUrl("http://127.0.0.2:8008", LOCAL_POLICY), "http://127.0.0.2:8008");
  assert.equal(assertAllowedHomeserverUrl("http://[::1]:8008", LOCAL_POLICY), "http://[::1]:8008");
...
test("legacy SSO callbacks are bound to a fresh initiating transaction", () => {
  ...
  assert.doesNotThrow(() => validateLegacySsoCallback(pending, callback, pending.createdAt + 60_000));
  assert.throws(
    () => validateLegacySsoCallback(pending, new URLSearchParams({ loginToken: "login-token" }), pending.createdAt + 60_000),
    /state/i,
  );
  assert.throws(
    () => validateLegacySsoCallback(pending, new URLSearchParams({
      loginToken: "login-token",
  ...
  const redirect = new URL(legacySsoRedirectUrl("https://sub-etha-matrix.vercel.app", pending.state));
...
test("invalid and crossed auth callbacks clear state and URL data before token exchange", async () => {
  ...
  const installCallback = (search: string, hash = "") => {
    Object.defineProperty(globalThis, "window", {
      ...
      value: {
        ...
        history: { replaceState: (_state: unknown, _unused: string, path: string) => replaced.push(path) },
      },
    });
  ...
  try {
    ...
    values.set("sub-etha-pending-auth", JSON.stringify({
      kind: "sso",
      baseUrl: "https://matrix.example",
      state: "expected",
      createdAt: Date.now(),
    ...
    if (hadSessionStorage) Object.defineProperty(globalThis, "sessionStorage", { configurable: true, value: originalSessionStorage });
...
test("OAuth metadata and authorization navigation reject unsafe URL schemes", async () => {
  ...
  assert.doesNotThrow(() => auth.assertSafeOAuthMetadata?.({
  ...
  for (const unsafe of [
    ...
    assert.throws(() => auth.assertSafeOAuthNavigationUrl?.(unsafe), /OAuth|HTTPS|credentials/i);
  }
  assert.equal(
  ...
  assert.throws(
    () => auth.assertSafeSsoNavigationUrl?.("https://attacker.example/login", "https://matrix.example"),
    /SSO/i,
  );
});

test("malformed room URL fragments are ignored without throwing", async () => {
  const chatShell = await import("../app/components/ChatShell") as typeof import("../app/components/ChatShell") & {
  ...
  assert.equal(chatShell.parseRoomHash?.("#/room/%21safe%3Amatrix.example"), "!safe:matrix.example");
});
...
test("only Matrix content URIs qualify as remote avatars", () => {
  ...
  const externalAvatar = renderToStaticMarkup(createElement(Avatar, {
    name: "Ford Prefect",
...
test("media downloads enforce declared, header, and streamed byte limits", async () => {
  assert.throws(
    () => assertDeclaredMediaLimits({ size: MAX_MEDIA_BYTES + 1 }),
    MediaLimitError,
  );
...
test("stalled media streams time out, cancel, and remain retriable", async () => {
  ...
  const result = await Promise.race([
    ...
    ) => Promise<ArrayBuffer>)(new Response(body), 4, { idleTimeoutMs: 10, totalTimeoutMs: 25 })
...
test("image previews validate actual dimensions rather than declared MIME metadata", () => {
  ...
  assert.throws(() => assertSafeImageBytes(new TextEncoder().encode("not an image")), MediaLimitError);
});
...
test("image previews bound decoded bytes and cumulative animation work", () => {
  ...
  const animatedGif = Uint8Array.from([
    71, 73, 70, 56, 57, 97,
    0, 16, 0, 16, 0, 0, 0,
    44, 0, 0, 0, 0, 0, 16, 0, 16, 0, 2, 2, 76, 1, 0,
    44, 0, 0, 0, 0, 0, 16, 0, 16, 0, 2, 2, 76, 1, 0,
    59,
  ]);
...
test("forged image MIME metadata cannot bypass byte inspection", async () => {
  ...
  globalThis.fetch = (async () => new Response(new TextEncoder().encode("not an image"), {
    headers: { "Content-Type": "image/png" },
  })) as typeof fetch;
  const service = mediaService();
  try {
    await assert.rejects(service.getMediaAsset(
...
function mediaService(): MatrixService {
  ...
  const client = {
    ...
    mxcUrlToHttp: (mxcUrl: string) => `https://matrix.example/media/${encodeURIComponent(mxcUrl)}`,
  };
...
test("automatic Matrix media loading never exceeds three concurrent fetches", async () => {
  ...
  const service = mediaService();
  try {
    const loads = Array.from({ length: 4 }, (_, index) => service.getMediaAsset(
  ...
  try {
    for (let index = 0; index < 5; index += 1) {
      internal.mediaAssets.set(`asset-${index}`, {
        promise: Promise.resolve({ url: `blob:large-${index}`, blob: new Blob(), mimeType: "application/octet-stream", animated: false }),
...
test("evicting or shutting down media work aborts pending downloads", async () => {
  ...
  const service = mediaService();
  try {
    const loads = Array.from({ length: 97 }, (_, index) => service.getMediaAsset(
