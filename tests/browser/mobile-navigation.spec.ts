...
test("edge swipes open and close the room index while vertical scrolling wins", async ({
    ...
    await expect(shell).toHaveClass(/mobile-rooms-open/);
    await expect(page).not.toHaveURL(/#\/room\//);
    await page.waitForTimeout(250);

    await dispatchTouchSwipe(page, { x: 340, y: 110 }, { x: 220, y: 114 });
    await expect(shell).not.toHaveClass(/mobile-rooms-open/);
...
test("reduced motion removes drawer and dialog transition timing", async ({ page }) => {
    ...
    const durations = await Promise.all(
        [page.locator(".room-sidebar"), page.locator(".sidebar-scrim")].map((locator) =>
