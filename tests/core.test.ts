...
import {
    ...
    mediaAuthorizationHeaders,
    shouldTryLegacyMedia,
...
test("homeserver input accepts Matrix IDs, domains, and explicit URLs", () => {
    assert.deepEqual(normalizeHomeserverInput("@arthur:matrix.example"), {
...
test("authentication errors are direct and callback credentials are removed", () => {
    assert.equal(
        humanizeMatrixError({ errcode: "M_FORBIDDEN" }),
        "The homeserver declined those credentials.",
    );
    assert.equal(sanitizedCallbackPath("/", "#code=private&state=private"), "/");
    assert.equal(
        sanitizedCallbackPath("/", "#/room/%21safe%3Aexample"),
        "/#/room/%21safe%3Aexample",
    );
});
...
test("room sorting prioritizes invites, favourites, unread rooms, then recency", () => {
    ...
    assert.deepEqual(
        sorted.map(({ id }) => id),
        ["invite", "favourite", "unread", "new", "old"],
    );
});
...
test("authenticated media credentials are restricted to the homeserver origin", () => {
    assert.deepEqual(
        mediaAuthorizationHeaders(
            "https://matrix.example/_matrix/client/v1/media/download/a/b",
    ...
    assert.throws(
        () =>
            mediaAuthorizationHeaders(
                "https://media.attacker.example/image",
                "https://matrix.example",
                "secret",
            ),
    ...
    assert.equal(
        mediaAuthorizationHeaders("https://matrix.example/image", "https://matrix.example", null),
        undefined,
    );
...
test("emoji insertion replaces the active composer selection and returns the new caret", () => {
    assert.deepEqual(insertAtSelection("Hello world", "🛰️", 6, 11), {
        value: "Hello 🛰️",
        caret: 9,
    });
});
...
test("encrypted events expose pending, failed, and ready decryption states", () => {
    ...
    assert.equal(
        eventDecryptionState(
            event({ type: "m.room.encrypted", decrypting: true, failed: false, encrypted: true }),
        ),
        "decrypting",
    );
    assert.equal(
        eventDecryptionState(
            event({ type: "m.room.encrypted", decrypting: false, failed: true, encrypted: true }),
        ),
        "failed",
    );
    assert.equal(
        eventDecryptionState(
            event({ type: "m.room.message", decrypting: false, failed: false, encrypted: true }),
        ),
        "ready",
    );
    assert.equal(
        eventDecryptionState(
            event({ type: "m.room.message", decrypting: false, failed: false, encrypted: false }),
        ),
        "ready",
    );
});
...
test("browser push reconciliation repairs missing and mismatched application keys", () => {
    const matching = {
        options: { applicationServerKey: Uint8Array.from([1, 2, 3]).buffer, userVisibleOnly: true },
    };
    const mismatched = {
        options: { applicationServerKey: Uint8Array.from([1, 2, 4]).buffer, userVisibleOnly: true },
    };
...
test("message content carries Matrix mentions, replies, and edits", () => {
    ...
    assert.deepEqual((edit["m.new_content"] as Record<string, unknown>)["m.relates_to"], {
...
test("plain message text recognizes links without swallowing sentence punctuation", () => {
    assert.deepEqual(
        messageTextSegments(
            "See https://example.org/docs_(v2), www.matrix.org or crew@example.org.",
        ),
        [
            { text: "See " },
    ...
    assert.equal(
        stripPlainReplyFallback("> <@ford:example.org> Earlier\n> message\n\nActual reply"),
        "Actual reply",
    );
