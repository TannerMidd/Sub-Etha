...
test("Neon registers, deduplicates, and deletes push subscriptions", {
  ...
  try {
    assert.equal(await registerConfirmedSubscription(
      pushKeyHash,
      managementKeyHash,
    ...
    assert.equal((await neonPushRepository.getManagedSubscription(managementKeyHash))?.deliveryKeyHash, pushKeyHash);
...
test("Neon atomically deduplicates endpoints and enforces capacity", {
  skip: !process.env.DATABASE_URL,
}, async () => {
  const suffix = crypto.randomUUID().replaceAll("-", "");
  ...
  try {
    ...
    assert.equal((await neonPushRepository.getSubscription(secondKey))?.endpoint, sharedEndpoint);

    const [state] = await getDb().select({ count: pushGatewayState.subscriptionCount })
      .from(pushGatewayState).where(eq(pushGatewayState.id, 1)).limit(1);
    const maximum = Number(state?.count ?? 0) + 1;
    const starts = await Promise.all([
      neonPushRepository.beginSubscriptionRegistration(
        capacityKeyA,
        capacityManagementA,
        { endpoint: `https://push.example.invalid/capacity-a-${suffix}`, p256dh: "capacity-a-p256dh", auth: "capacity-a-auth" },
      ...
      neonPushRepository.beginSubscriptionRegistration(
        capacityKeyB,
        capacityManagementB,
        { endpoint: `https://push.example.invalid/capacity-b-${suffix}`, p256dh: "capacity-b-p256dh", auth: "capacity-b-auth" },
    ...
    const outcomes = await Promise.all([
      neonPushRepository.confirmSubscription(`integration-capacity-challenge-a-${suffix}`, now + 2, maximum),
      neonPushRepository.confirmSubscription(`integration-capacity-challenge-b-${suffix}`, now + 2, maximum),
    ]);
...
test("Neon global budgets reset atomically and stale cleanup reconciles count", {
  skip: !process.env.DATABASE_URL,
}, async () => {
  const suffix = crypto.randomUUID().replaceAll("-", "");
  ...
  const [budgetBefore] = await getDb().select({
    windowStart: pushGlobalRateBudgets.windowStart,
    requestCount: pushGlobalRateBudgets.requestCount,
  }).from(pushGlobalRateBudgets).where(eq(pushGlobalRateBudgets.name, "test-sends")).limit(1);
  const [before] = await getDb().select({ count: pushGatewayState.subscriptionCount })
    .from(pushGatewayState).where(eq(pushGatewayState.id, 1)).limit(1);

  try {
    await getDb().update(pushGlobalRateBudgets).set({
      windowStart: budgetNow - 61,
      requestCount: 0,
    }).where(eq(pushGlobalRateBudgets.name, "test-sends"));
    assert.equal(await neonPushRepository.consumeGlobalRateLimit("test-sends", budgetNow, 60, 1), true);
    assert.equal(await neonPushRepository.consumeGlobalRateLimit("test-sends", budgetNow, 60, 1), false);
    assert.equal(await neonPushRepository.consumeGlobalRateLimit("test-sends", budgetNow + 61, 60, 1), true);
    ...
    assert.equal(await neonPushRepository.getSubscription(pushKeyHash), null);
    const [after] = await getDb().select({ count: pushGatewayState.subscriptionCount })
      .from(pushGatewayState).where(eq(pushGatewayState.id, 1)).limit(1);
    assert.equal(after?.count, before?.count);
  } finally {
    await neonPushRepository.deleteSubscription(managementKeyHash);
    if (budgetBefore) {
      await getDb().update(pushGlobalRateBudgets).set(budgetBefore)
        .where(eq(pushGlobalRateBudgets.name, "test-sends"));
    }
  }
});
